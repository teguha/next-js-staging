                ...
                <div className="p-6">
                    ...
                    <div className="mb-5">
                        <div className="flex items-center justify-center gap-2 mb-1">
                            {oldPrice && <span className="text-sm line-through text-gray-400">{oldPrice.toLocaleString()}</span>}
                            {discount && <span className="bg-yellow-400 text-xs font-bold px-2 py-0.5 rounded-full text-gray-900">{discount} OFF</span>}
                        </div>
                        <p className="text-center">
                            <span className="text-3xl font-bold text-black">{newPrice.toLocaleString()}</span>
                            <span className="text-sm text-gray-500 px-2">/ PAX</span>
                        </p>
                    </div>

                    <div className="flex gap-3">
                    <button onClick={() => setOpen(true)} className="flex-1 text-center text-blue-600 hover:text-blue-700 font-medium py-2.5 border-2 border-blue-600 rounded-lg hover:bg-blue-50 transition-all">
                        Details
                    </button>
